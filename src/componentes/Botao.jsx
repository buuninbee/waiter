...
const variants = {
  ...
  'secundario': "bg-(--terciario-100) text-(--primario-700) border border-(--primario700) hover:bg-(--terciario-200) hover:text-(--primario-500)"
};
...
const Botao = ({text, path, variant = "primario", img}) => {
  return (
    <button className={cn(defaultButtonClasses, variants[variant])}>
        {
            img ? 
            <Link className="flex justify-center gap-2" to={path}>
